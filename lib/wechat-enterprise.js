...
var verifyAndGetMessage = function (req, res, callback) {
  load(req, function (err, buf) {
    ...
    xml2js.parseString(xml, {trim: true}, function (err, result) {
      ...
      var signature = wxcpt.getSignature(timeStamp, nonce, encryptMessage);
      if (signature !== msgSignature) {
        res.writeHead(401);
        res.end('Invalid signature');
        return;
      }
      var messageWrapXml = wxcpt.prpDecrypt(encryptMessage);
      if (messageWrapXml === '') {
        res.writeHead(401);
...
var respond = function (handler) {
  return function (req, res, next) {
    verifyAndGetMessage(req, res, function (err, result) {
      if (err) {
      ...
      res.reply = function (content) {
        ...
        var wrap = {};
        wrap.encrypt = wxcpt.prpEncrypt(replyMessageXml);
        wrap.nonce = parseInt((Math.random() * 100000000000), 10);
        wrap.timestamp = new Date().getTime();
        wrap.signature = wxcpt.getSignature(wrap.timestamp, wrap.nonce, wrap.encrypt);
...
Handler.prototype.middlewarify = function () {
  ...
  return function (req, res, next) {
    ...
    if (method === 'GET') {
      ...
      var signature = wxcpt.getSignature(timeStamp, nonce, echostr);
      if (signature !== msgSignature) {
        res.writeHead(401);
        res.end('Invalid signature');
        return;
      }
      var echoStr = wxcpt.prpDecrypt(echostr);
      res.writeHead(200);
      res.end(echoStr);
    } else if (method === 'POST') {
 ...
 * @param {Object} config 在微信平台填写的口令
