...
var WXBizMsgCrypt = require('./msg_crypto');
...
var compiledWrapTpl = ejs.compile(wrapTpl);
...
/*!
 * 验证调用者的合法性并从微信的提交中提取XML文件
 */
var verifyAndGetMessage = function (req, res, callback) {
  var that = this;
  load(req, function (err, buf) {
    ...
    xml2js.parseString(xml, {trim: true}, function (err, result) {
      ...
      var messageWrap = formatMessage(result.xml);
      var encryptMessage = messageWrap.Encrypt;
      var msgSignature = req.query.msg_signature;
      var timeStamp = req.query.timestamp;
      var nonce = req.query.nonce;
      var signature = that.wxcpt.getSignature(timeStamp, nonce, encryptMessage);
      if (signature !== msgSignature) {
        res.writeHead(401);
        res.end('Invalid signature');
        return;
      }
      var messageWrapXml = that.wxcpt.prpDecrypt(encryptMessage);
      if (messageWrapXml === '') {
        res.writeHead(401);
      ...
      xml2js.parseString(messageWrapXml, {trim: true}, callback);
    });
  });
};
      ...
      for (var key in result) {
        if (result[key].length === 1) {
          ...
          result[key].forEach(function (item) {
            message.push(formatMessage(item));
          });
        }
      }
  }
 ...
 * 将回复消息封装成xml格式
 */
var wrapReplyMessage2Xml = function (content, fromUsername, toUsername) {
  var info = {};
  var type = 'text';
...
var respond = function (handler) {
  var _verifyAndGetMessage = verifyAndGetMessage.bind(handler);
  return function (req, res, next) {
    _verifyAndGetMessage(req, res, function (err, result) {
      if (err) {
        err.name = 'BadMessage' + err.name;
        return next(err);
      }
      ...
      res.reply = function (content) {
        ...
        var replyMessageXml = wrapReplyMessage2Xml(content, message.ToUserName, message.FromUserName);
        var wrap = {};
        wrap.encrypt = handler.wxcpt.prpEncrypt(replyMessageXml);
        wrap.nonce = parseInt((Math.random() * 100000000000), 10);
        wrap.timestamp = new Date().getTime();
        wrap.signature = handler.wxcpt.getSignature(wrap.timestamp, wrap.nonce, wrap.encrypt);
        res.end(compiledWrapTpl(wrap));
      };
...
Handler.prototype.middlewarify = function () {
  ...
  this.wxcpt = new WXBizMsgCrypt(config.token, config.encodingAESKey, config.corpId);
  return function (req, res, next) {
    ...
    if (method === 'GET') {
      ...
      var signature = this.wxcpt.getSignature(timeStamp, nonce, echostr);
      if (signature !== msgSignature) {
        res.writeHead(401);
        res.end('Invalid signature');
        return;
      }
      var echoStr = this.wxcpt.prpDecrypt(echostr);
      res.writeHead(200);
      res.end(echoStr);
    } else if (method === 'POST') {
...
middleware.WXBizMsgCrypt = WXBizMsgCrypt;
