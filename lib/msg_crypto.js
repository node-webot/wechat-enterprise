...
PKCS7Encoder.decode = function (text) {
  var pad = text.slice(text.length - 1).toString().charCodeAt(0);
  if (pad < 1 || pad >32) {
    pad = 0;
  }
...
PKCS7Encoder.encode = function (text) {
  ...
  //获得补位所用的字符
  var pad = new Buffer(String.fromCharCode(amountToPad));

  var padResult = new Buffer('');
  for (var i = 0; i < amountToPad - 1; i++) {
    padResult = Buffer.concat([padResult, pad]);
  }

  return Buffer.concat([text, padResult]);
};
 ...
 */
WXBizMsgCrypt.prototype.prpDecrypt = function(text) {
  ...
  var msgLength = content.slice(0, 4).readUInt32BE(0);
  var msg = content.slice(4, msgLength + 4).toString();
  var corpIdFromXml = content.slice(msgLength + 4).toString();
  if (corpIdFromXml !== this.corpId) {
    // or throw Exception?
    return '';
  }
  return msg;
 ...
 */
WXBizMsgCrypt.prototype.prpEncrypt = function(text) {

  // 算法：AES_Encrypt[random(16B) + msg_len(4B) + msg + $CorpID]
  // 获取16B的随机字符串
  var randomString = new Buffer(16);
  randomString.write(getRandomString(16));
